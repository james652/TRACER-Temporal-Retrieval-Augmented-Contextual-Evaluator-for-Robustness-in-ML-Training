        ...
        self.prompt = ChatPromptTemplate.from_messages([
            (
                ...
                (
                    ...
                    "[PROMPT VARIANT: EVIDENCE-FIRST FORMULATION]\n"
                    "This prompt variant uses the same attack definitions and decision rules as the original prompt, "
                    "but changes the reasoning order to prioritize explicit evidence extraction before final classification. "
                    "First, identify numerical evidence from the execution logs, including task-wise accuracy changes, "
                    "BWT or forgetting trends, label-noise detection metrics, loss trajectories, and attack execution traces. "
                    "Second, compare the extracted evidence with the predefined attack rules. "
                    "Third, determine whether the identified attack type, rationale, and risk level remain consistent with the observed evidence. "
                    "Do not infer an attack solely from file names, script names, or scenario metadata unless they are supported "
                    "by numerical or log-level evidence.\n\n"
...
class Runner:
    ...
    - Instead of reading stdout/stderr line by line, uses
      subprocess.run(..., stdout=logf, stderr=STDOUT) to
      write directly to the log file only, reducing I/O overhead.
...
def llm_extract_topk(openai_client: Optional[OpenAI], model: str, text: str, k: int = 10) -> Dict[str, Any]:
    ...
    Falls back to a simple statistical extraction on failure.
    """
...
def monitor_pipeline(
    ...
    """
    For each step: run ➜ summarize/evaluate (Analysis method) ➜ (NEW: MMD PNG vision analysis) ➜
            ...
            try:
                ...
                if USE_VISION and is_mmd_pipeline and is_visual_step and analyzer.client is not None:
                    ...
                    if png_path:
                        ...
                        # Append to summary to use as "decision evidence"
                        summary_text = (
                            summary_text
...
def gpt_analyze_png(openai_client: Optional[OpenAI], model: str, png_path: str) -> Dict[str, Any]:
    ...
    resp = openai_client.responses.create(
        ...
        # optional: add options such as temperature=0 if needed
    )
