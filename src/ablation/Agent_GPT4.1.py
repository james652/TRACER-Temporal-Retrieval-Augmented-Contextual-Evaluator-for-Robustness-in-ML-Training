...
class RAGRetriever:
    ...
    def __init__(self, openai_client: Optional[OpenAI], model: str = "gpt-4.1"):
...
class Summarizer:
    ...
    def __init__(self, memory_mgr: MemoryManager, model: str = "gpt-4.1", temperature: float = 1.0):
...
class RAGASEvaluator:
    ...
    @staticmethod
    def _safe_num(s: str, default: Optional[float] = None) -> Optional[float]:
        # Pull the first number out of the judge response instead of requiring a
        # pure float. Judge replies like "0.85.", "Score: 0.9", or a value wrapped
        # in markdown/newlines previously fell through to 0.0 -> silent false zeros.
    ...
    def evaluate(self, question: str, context: str, answer: str) -> Dict[str, Any]:
        ...
        try:
            ...
            val = self._safe_num(f_resp.choices[0].message.content)
            if val is None:
                out["faithfulness_error"] = (
                    "judge returned non-numeric output: "
                    f"{(f_resp.choices[0].message.content or '')[:80]!r}"
                )
            else:
        ...
        try:
            ...
            val = self._safe_num(cr_resp.choices[0].message.content)
            if val is None:
                out["context_relevance_error"] = (
                    "judge returned non-numeric output: "
                    f"{(cr_resp.choices[0].message.content or '')[:80]!r}"
                )
            else:
...
class Runner:
    ...
    - Instead of reading stdout/stderr line by line, use
      subprocess.run(..., stdout=logf, stderr=STDOUT) to write directly
      to the log file only, reducing I/O overhead.
...
def llm_extract_topk(openai_client: Optional[OpenAI], model: str, text: str, k: int = 10) -> Dict[str, Any]:
    """
    Use the LLM to extract the top-K terms/phrases from the log/summary text for RAG retrieval.
    On failure, fall back to a simple statistical extraction.
    """
...
# ====================================================================================
class Analyzer:
    def __init__(self, log_dir: str, model: str = "gpt-4.1"):
...
# ====================================================================================
def gpt_verify_step(step_log_path: str, step_title: str, model: str = "gpt-4.1") -> Dict[str, Any]:
...
""".strip()
    ...
    try:
        ...
        summary_guess = ("Log tail:\n" + "\n".join(last_lines[:5])) if last_lines else "Summary unavailable (failed to parse model response)"
        data = {
            "step_executed": None,
...
def build_report(steps: List[StepRunResult], gpt_model: str = "gpt-4.1") -> Dict[str, Any]:
...
def monitor_pipeline(
    ...
    for idx, spec in enumerate(specs, start=1):
        ...
        if use_langchain_analysis:
            ...
            # ------------------------------------------------------------------
            #  (NEW) For an MMD Visualization step, analyze the PNG again with GPT Vision
            # ------------------------------------------------------------------
            try:
            ...
            # 5) RAGAS  (compute scores using the vision-merged summary_text)
            try:
                question = (spec.rag_request or spec.analysis_prompt or f"What happened in step: {spec.title}?")[:1000]
                context = f"rag_answer: {rag_answer}\n\nlog_text: {log_text}"
                answer = summary_text[:4000]
            ...
            # 6) Attach to result  (save the vision-reflected summary_text)
            ...
            if USE_TOPK:
                ...
                # Update rag_request if there is a next step
                if idx < len(specs):
                    next_spec = specs[idx]  # next one in 0-based index
...
def gpt_analyze_png(openai_client: Optional[OpenAI], model: str, png_path: str) -> Dict[str, Any]:
    ...
    resp = openai_client.responses.create(
        ...
        # Options like temperature=0 can be added if needed
    )

    # Use output_text if the SDK provides it, otherwise treat raw as a string
    text = getattr(resp, "output_text", None) or ""
    # Parse assuming text is a JSON string
    try:
        obj = json.loads(text)
...
if __name__ == "__main__":
    ...
    choice = input("Which program should be run? (Brainwash / brainwash_miniimagenet/ brainwash_cifar10 / accumulative_cifar100/ brainwash_tinyimagenet / Accumulative / Test / Analysis / Analysis_Accumulative  / MMD_backdoor / MMD_backdoor_cifar100 / Detect / Rethink/ Rethink_pub) [Brainwash]: ").strip() or "Brainwash"
    ...
    if choice.lower() in ("analysis", "analyze", "a"):
        ...
        analyzer = Analyzer(LOG_DIR, model="gpt-4.1")
        ...
        report = build_report(results, gpt_model="gpt-4.1")
