...
def _merge_rag_request(existing: str, topk_obj: Dict[str, Any], max_len: int = 512) -> str:
    """
    Combine existing + (terms + phrases + query_hint), then dedup/trim.
    """
...
def llm_extract_topk(openai_client: Optional[OpenAI], model: str, text: str, k: int = 10) -> Dict[str, Any]:
    """
    Use the LLM to extract the top-K terms/phrases from log/summary text for RAG retrieval.
...
class Analyzer:
    ...
    def analyze(self, specs: List[StepSpec], session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        TOPK_K = int(os.getenv("TOPK_K", "1"))  # query with a single Top-K clue by default
...
""".strip()
    ...
    try:
        ...
        summary_guess = ("Log tail:\n" + "\n".join(last_lines[:5])) if last_lines else "Unable to summarize (model response parsing failed)"
        data = {
            "step_executed": None,
...
def monitor_pipeline(
    ...
    fallback_gpt_verify: bool = False,    # legacy verification when needed
) -> List[StepRunResult]:
    """
    ...
    for idx, spec in enumerate(specs, start=1):
        ...
        if use_langchain_analysis:
            ...
            # 6) Attach to result  (store the vision-reflected summary_text)
            ...
            if USE_TOPK:
                ...
                if idx < len(specs):
                    next_spec = specs[idx]  # next item in 0-based index
...
def gpt_analyze_png(openai_client: Optional[OpenAI], model: str, png_path: str) -> Dict[str, Any]:
    """
    Feed the PNG as a vision input and receive a backdoor/clean/uncertain verdict as JSON.
    """
    if openai_client is None:
