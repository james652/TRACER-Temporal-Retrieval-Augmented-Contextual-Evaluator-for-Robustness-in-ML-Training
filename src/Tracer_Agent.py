...
from attack_spec import (
    ...
    build_fgsm_specs,
    build_pgd_specs,
    build_physpatch_specs,
)

# ------------------------------------------------------------------------------------
# Detection system prompt (externalized to keep this file readable)
# ------------------------------------------------------------------------------------
from detection_prompt import DETECTION_SYSTEM_PROMPT
...
def llm_extract_topk(openai_client: Optional[OpenAI], model: str, text: str, k: int = 10) -> Dict[str, Any]:
    """
    Use the LLM to extract the top-K terms/phrases from log/summary text for RAG search.
...
def monitor_pipeline(
    ...
    """
    For each step: execute ➜ summarize/evaluate (Analysis-style) ➜ (NEW: MMD PNG vision analysis) ➜
    ...
    for idx, spec in enumerate(specs, start=1):
        ...
        if use_langchain_analysis:
            ...
            # ------------------------------------------------------------------
            #  (NEW) For an MMD Visualization step, analyze the PNG once more with GPT Vision
            # ------------------------------------------------------------------
            try:
                ...
                if USE_VISION and is_mmd_pipeline and is_visual_step and analyzer.client is not None:
                    ...
                    if png_path:
                        ...
                        # Merge into summary to use as the "decision basis"
                        summary_text = (
                            summary_text
            ...
            # 5) RAGAS  (score using the vision-merged summary_text)
            try:
                question = (spec.rag_request or spec.analysis_prompt or f"What happened in step: {spec.title}?")[:1000]
                context = f"rag_answer: {rag_answer}\n\nlog_text: {log_text}"
                answer = summary_text[:4000]
...
if __name__ == "__main__":
    ...
    choice = input("Which program would you like to run? (Brainwash / brainwash_miniimagenet/ brainwash_cifar10 / accumulative_cifar100/ brainwash_tinyimagenet / Accumulative / Test / Analysis / Analysis_Accumulative  / MMD_backdoor / MMD_backdoor_cifar100 / Detect / Rethink/ Rethink_pub / FGSM / PGD / PhysPatch) [Brainwash]: ").strip() or "Brainwash"
    ...
    if choice.lower() in ("analysis", "analyze", "a"):
        ...
        build = {
            ...
            "fgsm": build_fgsm_specs,
            "pgd": build_pgd_specs,
            "physpatch": build_physpatch_specs,
